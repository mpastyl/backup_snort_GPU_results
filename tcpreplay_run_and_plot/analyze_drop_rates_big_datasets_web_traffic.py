...
for v in versions:
    for s in speeds:
        l = []
        for rep in repeat:
            name = "/Users/mpastyl/snort_GPU_plots/logs_from_snort/result_proper_big_dataset_proper_config_web_stream_"+str(v)+"_"+str(s)+"_"+str(rep)+".txt"
            switch=1
...
ax.plot(speeds,Data['0'], label='Snort modified (CPU)',color=colors[1])
ax.plot(speeds,Data['3'], label='Snort original',color=colors[0])
ax.plot(speeds,Data['1'], label='CLort single buffer (GPU)', color=colors[2])
ax.plot(speeds,Data['2'], label='CLort double buffer (GPU)', color=colors[3])
lgd = plt.legend()
